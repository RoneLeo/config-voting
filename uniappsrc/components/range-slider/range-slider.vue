<template>
	<view class="range-slider" :style="'width:'+width+';height:' + height + 'rpx'">
		<view class="range-bar" :style="'width:100%;height:' + barHeight + 'rpx'">
			...
			<view class="range-bar-progress" :style="'width:' + progressBarWidth + 'rpx;background-color:' + activeColor + ''"></view>
		</view>
	...
	methods: {
		...
		_onBlockTouchMove: function(e) {
			...
			if (tag == 'minBlock' || tag == 'maxBlock') {
				...
				var eventOption = {};
				that.$emit('range', Math.floor(eventDetail.maxValue));
