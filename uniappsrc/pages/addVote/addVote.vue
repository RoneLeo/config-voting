	...
	export default {
		...
		onLoad(params) {
			...
			if(params.hdid || this.hdid) {
				this.$api.post('/theme/findAllInfoById', {id: params.hdid || this.hdid}).then((res) => {
					if(res.resCode == 200) {
		...
		onShow() {
			if(this.paramsHdid || this.hdid) {
				this.$api.post('/theme/findAllInfoById', {id: this.paramsHdid || this.hdid}).then((res) => {
					if(res.resCode == 200) {
						this.activityForm = res.data;
		...
		methods: {
			...
			saveTheme() {
				...
				for(let key in this.activityForm) {
					if(key != 'scorelist' && key != 'scoreoblist') {
...
.cu-form-group .title {
	...
	.vote-list {
		...
		.vote-item {
			...
			.vote-index, .vote-item-bt {
				color: #555;
			}
			.vote-item-bt, .vote-item-opt {
				line-height: 2;
			}
			.vote-item-opt {
				display: flex;
